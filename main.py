...
@app.route("/")
def hello_world():
    return f'<h1>технологические зависимости</h1> <a href="/facts">перейти к фактом</a>'

@app.route("/secret")
def secret():
    return f'<h1> -где зависают старые качки бодибилдеры?    -в кресле качалке</h1> <a href="/"вернуться на главную</a>'


@app.route("/facts")
def new_facts():
    return f'<h1>факты</h1> <p>{random.choice(facts)}</p> <a href="/secret">анектод</a>'
